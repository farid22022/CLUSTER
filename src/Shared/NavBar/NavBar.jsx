...
const NavBar = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all  duration-300 ${scrolled ?  'bg-linear-to- to to-blue-800  backdrop-blur-sm py-4':'bg-linear-to-r white to to-blue-800 shadow-lg py-2' }`}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center">
            ...
            />
            <span className="text-xl md:text-2xl font-bold tracking-tight">CLUSTER</span>
          </div>
          ...
          <div className="hidden lg:flex space-x-1">
            {navLinks.map((link, index) => (
              <div 
                key={link.path}
                className="relative group text-blue-700 hover:text-white transition-colors"
        ...
        <div className={`lg:hidden ${isMenuOpen ? 'block' : 'hidden'} transition-all duration-300 overflow-hidden`}>
          <div className="pt-2 pb-4 space-y-1 bg-blue-800 rounded-lg mt-2 shadow-xl">
            {navLinks.map((link, index) => (
              <div key={link.path} className="border-b border-blue-700/50 last:border-0">
                ...
                >
                  ...
                  {link.submenu && (
                    ...
                    >
                      <svg 
                        className={`w-5 h-5 transform transition-transform ${activeDropdown === index ? 'rotate-180' : ''}`} 
                        fill="currentColor" 
                        viewBox="0 0 20 20"
                      >
                        <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
